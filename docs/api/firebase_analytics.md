...
- 请求方法：`POST`
- 请求路径：`/report/user-summary/query`
- 权限要求：`admin`
- 数据来源表：`firebase_report_user_summary`

查询结构与 `queryUserReportSummary` 一致，支持：

- `dateFrom` / `dateTo` / `hourFrom` / `hourTo`
- `groupBy` / `filters` / `page` / `pageSize` / `orderBy` / `orderDirection`
...
- `appIds`、`appVersions`、`platforms`、`countries`、`networkTypes`

### 31. 节点汇总查询

- 请求方法：`POST`
- 请求路径：`/report/node/query`
- 权限要求：`admin`
- 数据来源表：`firebase_report_node`
...
- `date`、`hour`、`app_id`、`app_version`、`country`、`node_id`、`node_host`、`node_name`、`node_country`、`node_region`、`protocol`

可用 `filters` 字段：

- `appIds`、`appVersions`、`countries`、`nodeIds`、`nodeHosts`、`nodeCountries`、`protocols`
