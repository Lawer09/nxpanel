...
- 不传或传空数组：明细，按 `reportDate + projectCode + country` 原始粒度返回。
- 传数组：按数组中的维度组合聚合，例如 `['reportDate', 'projectCode']` 或 `['reportDate', 'projectCode', 'country']`。
...
说明：该接口会同步调用 `project:aggregate-daily --start-date --end-date`；传入 `projectId` 时会追加 `--project-id`，命令只删除和重建该项目的日报聚合结果。`project_report_hourly` 由独立命令 `project:aggregate-hourly` 和管理端 `/projects/aggregate-hourly` 接口维护。
...
- 该接口仅投递队列任务并立即返回。
- 传入 `projectId` 时，队列任务会透传 `project:aggregate-daily --project-id`。
- 需确保队列消费者已启动，例如 `php artisan queue:work`。
