...
## 2026-06-03 广告花费分页拉取停止条件调整

- 日期：2026-06-03
- 变更摘要：`AdSpendPlatformService::fetchDailyRecords` 改为按页拉取，直到当前页返回空数据后停止；同时新增最大页数保护，防止远端分页异常导致死循环。
- 影响范围：`app/Services/AdSpendPlatformService.php`
- 是否需要迁移/回滚说明：无需数据库迁移；如需回滚，恢复 `fetchDailyRecords` 的当前分页终止与保护逻辑即可。

## 2026-06-03 WooCommerce 回执状态流转修正

- 日期：2026-06-03
- 变更摘要：调整 WooCommerce 回执处理逻辑，`trigger=processing` 仅记录为支付中并创建本地待支付订单，`trigger=completed` 才触发本地支付/开通流程。
- 影响范围：`app/Services/WooCommerceOrderReceiptService.php`、`tests/Feature/WooCommerceOrderPaidTest.php`、`docs/api/woocommerce_order_api.md`、`docs/api/external_order_receipt_api.md`
- 幂等说明：同一个 `order.order_id` 可从 `pending` 推进到 `processed`；已 `processed` 的重复推送仍返回 `duplicate=true` 且不会重复开通。
- 是否需要迁移/回滚说明：无需数据库迁移；如需回滚，恢复 `processing` 直接调用本地支付流程的旧逻辑即可。
