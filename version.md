...
## 2026-07-06 工单 latest_message 字段稳定返回
- 日期：2026-07-06
- 变更摘要：用户端 `/ticket/fetch` 详情查询同步预加载 `latestMessage`，并调整 `TicketResource` 使 `latest_message` 字段在未加载或无消息时稳定返回 `null`，避免字段被 Laravel Resource 过滤移除。
